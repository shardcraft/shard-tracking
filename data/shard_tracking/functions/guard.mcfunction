...
# Teleport all players in spawn that are under y=0 to spawn.
execute as @p[scores={shard=1,positionY=..32}] run function shard_tracking:spawn_guard
...
gamemode survival @a[nbt={Dimension: "shardcraft:bazmazan"},gamemode=adventure,scores={shard=17..}]
gamemode survival @a[nbt={Dimension: "shardcraft:aynkar"},gamemode=adventure,scores={shard=17..}]
